...
----------------------------------
After you make changes to the code, you should update the documentation. The
docs are built using `Sphinx <https://www.sphinx-doc.org/en/master/>`_ and
`sphinx-apidoc <https://www.sphinx-doc.org/en/master/man/sphinx-apidoc.html>`_.
You can build the documentation locally by typing:
...
open the ``index.html`` file in your web browser to view the docs. The
files in the ``_build`` directory are not tracked by git, but files 
in ``docs/source`` (generated by the ``sphinx-apidoc`` command) *are* tracked
by git, so make sure to check those in.
